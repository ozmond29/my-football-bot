...
def calculate_advanced_markets(home_attack, home_defense, away_attack, away_defense):
    ...
    # Generate the baseline probability distributions up to 2 goals
    p_home = [poisson_probability(i, lambda_home) for i in range(3)]
    p_away = [poisson_probability(i, mu_away) for i in range(3)]
    
    # 1. OVER 2.5 GOALS CALCULATION WITH TAULOCO ADJUSTMENTS
...
def get_fallback_fixtures():
    return [
        {"league": "Club Friendly", "home": "Bayern Munich", "away": "FC Basel", "h_att": 2.80, "h_def": 1.10, "a_att": 1.40, "a_def": 2.90},
        {"league": "USL League Two", "home": "City SC", "away": "AMSG FC", "h_att": 2.00, "h_def": 2.00, "a_att": 1.13, "a_def": 2.75},
        {"league": "International Friendly", "home": "Brazil", "away": "Germany", "h_att": 2.10, "h_def": 1.20, "a_att": 1.90, "a_def": 1.40}
    ]
...
def run_predictions():
    ...
    print(f"🏁 Execution finished. Dispatched {alerts_triggered} multi-market green light cards.")
