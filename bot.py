...
def get_live_fixtures():
    """Loops through the 12 free tier competitions allowed by football-data.org to get today's real games."""
    # List of safe, high-scoring free-tier competition codes
    free_competition_codes = ["PL", "PD", "BL1", "SA", "FL1", "DED", "PPL", "CL"]
    headers = {'X-Auth-Token': API_FOOTBALL_KEY}
    formatted_list = []
    
    print("⏳ Scanning free-tier competitive global leagues for active matches...")
    
    for code in free_competition_codes:
        # Querying the exact match schedules for each individual free league code
        ...
        try:
            ...
            # Filter specifically for games scheduled to start today
    ...
    print(f"📡 API Check Complete. Found {len(formatted_list)} actual games scheduled to play today inside free channels.")
    return formatted_list

def run_predictions():
    fixtures = get_live_fixtures()
    if not fixtures:
        print("🏁 Scan finished. No active games scheduled today inside your free data competitions.")
        return
    ...
    for item in fixtures:
        ...
        if any(blacklisted_league in league_name for blacklisted_league in LEAGUE_BLACKLIST):
            continue  
        ...
        if metrics["over_25_prob"] >= PROBABILITY_THRESHOLD or metrics["btts_prob"] >= PROBABILITY_THRESHOLD:
            ...
            print(f"✅ DUAL ALERT SENT: {home_team} vs {away_team} (O2.5: {metrics['over_25_prob']}% | {metrics['btts_prob']}%)")
            alerts_triggered += 1
            time.sleep(1)
            
    print(f"🏁 Execution finished. Dispatched {alerts_triggered} real value lines.")
